function Fig_Slope11() % MATLAB R2017a
close all;
...
rel_temp =((EXP.Session * EXP.SessionID * ANL.ProjTrialAverageBaseline11 * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction  * ANL.SessionGrouping * ANL.TrialTypeStimTime)) & k & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0') -  ANL.ExcludeSession;
...
key_trial_type.trial_type_name='l_-1.6Full';
line_style='-';
[xl, ~] =figure_plotModes_correct_or_error_timeshifted (rel_choice_full ,  Param, xlabel_flag, title1, title2, title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, k,[],line_style,0);
yl=[ -0.4 1];
set(gca,'YLim',yl,'Ytick',[-0.4  0 1]);
% text(xl(1)-diff(xl)*0.4 , yl(1)+diff(yl)*1.25, 'h', ...
%     'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
text(-1.5, y_lims(2)*1.5, sprintf('....  Early Delay\n     Error trials'), 'Color', [0.5 0.5 0.75], 'FontSize',7,'HorizontalAlignment','center')
box off;

set(gca,'XLim',[ -2.7 -0.9],'Xtick',[ -2.5 -0.9],'XtickLabel',[ 0 1.6],'FontSize',6);
...
rel  = (ANL.ProjTrialAverageBaseline11*EXP.SessionTraining* ANL.SessionGrouping *EXP.SessionID  & k)& (ANL.IncludeSessionSelectivity2 & 'num_sample_selective_units>=0') ;
...
for i_p = 1:1:size(proj,1)
    ...
    y1= mean(y(time>=t1 & time<t1+twind));
    dt1=mean(time((time>=t1 & time<t1+twind)));

    y2= mean(y(time>=t2-twind & time<t2));
    dt2=mean(time((time>=t2-twind & time<t2)));

    slope1(i_p)=(y2-y1)./(dt2-dt1);
...
for i_p = 1:1:size(proj,1)
    ...
    y1= mean(y(time>=(t1+tshift) & time<(t1+ tshift +twind)));
        dt1=mean(time((time>=(t1+tshift) & time<(t1+ tshift +twind))));

    y2= mean(y(time>=(t2+tshift-twind) & time<(t2+tshift)));
        dt2=mean(time((time>=(t2+tshift-twind) & time<(t2+tshift))));

    slope2(i_p)=y2-y1./(dt2-dt1);
    plot(time-tshift  +2.5,y,'-r', 'Color', [0.5 0.5 0.75],'LineWidth',2)
    %     xlim([0,2.5]);
end
...
[h,p]=ttest2(slope1,slope2)

p = signrank(slope1,slope2) % Wilcoxon signed rank test (paired-samples)
...
bar([2],[y_mean(2)],'facecolor',[0.5 0.5 0.75])
