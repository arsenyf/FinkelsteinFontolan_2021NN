function Sup_REV1_ChoiceMode_RightDistractors_ministimuli() % MATLAB R2017a
close all;

dir_save_figure = ['D:\Arseny\S1ALM_revision_figures\'];
filename = 'Sup_REV1_ChoiceMode_RightDistractors_ministimuli';
...
%--------------------------------------
% b_query= (EXP.SessionID * ANL.SessionBehavPerformance2 * EXP.SessionTraining * EXP.SessionTask * EXP.SessionComment  & 'task_protocol in (4,6,3,2,5,7,8,9)' & 'training_type="distractor"' & 'session_comment="good behavior"');
% b_query= (EXP.SessionID * ANL.SessionBehavPerformance2 * EXP.SessionTraining * EXP.SessionTask * EXP.SessionComment  & 'task_protocol in (4,6)' & 'training_type="distractor"' & 'session_comment="good behavior"');
b_query = EXP.SessionID * ANL.SessionBehavPerformance2  & (EXP.SessionTraining * (EXP.SessionTask & 'task="s1 stim"') & 'training_type="regular"'  & 'task_protocol in (2,5,7,8,9)' ... 
    & (ANL.SessionBehavPerformance2   & 'trial_type_name="r"' & sprintf('total_noignore_noearly>=%d',mintrials_behav_typeoutcome)) ...
    & (ANL.SessionBehavPerformance2 & 'trial_type_name="r_-1.6Mini"' & sprintf('total_noignore_noearly>=%d',mintrials_behav_typeoutcome)) ...
    & (ANL.SessionBehavPerformance2 & 'trial_type_name="r_-0.8Mini"' & sprintf('total_noignore_noearly>=%d',mintrials_behav_typeoutcome)) & EPHYS.Unit) ;

clear trial_type_names  b
[trial_type_names, b] = fn_fetch_behav_mini_rev(b_query);
names_right_trials = {'r', 'r_-3.8Mini', 'r_-1.6Mini', 'r_-0.8Mini'};
% names_left_trials = {'l', 'l_-3.8Full','r', 'l_-1.6Full', 'l_-0.8Full'};
names_left_trials = {'l', 'r', 'l_-1.6Mini', 'l_-0.8Mini'};
x_r = [(-3.8), (-1.6), (-0.8)];
% x_l = [(-3.8),(-2.5), (-1.6), (-0.8)];
x_l = [(-2.5), (-1.6), (-0.8)];
...
[xl,yl]=figure_plot_behav_distractor_mini_righttrials_bar (behav_param,behav_param_mean, behav_param_signif,  x_r, x_l, y_r, y_l, trn_r, trn_l, flag_plot_left_right_trials,plot_signif_offset, names_right_trials, names_left_trials, legends{2},trial_type_names );
...
k.trialtype_flag_right_early_delay_and_control_pure=1;
...
line_style='-';
xl=[-4,0];
[xl, yl] =figure_plotModes_correct_or_error_double_distractor (rel_choice_full,  Param, xlabel_flag, title1, title2, title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, k, xl,line_style,0);
