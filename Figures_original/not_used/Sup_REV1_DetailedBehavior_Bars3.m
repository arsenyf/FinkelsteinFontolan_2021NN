function Sup_REV1_DetailedBehavior_Bars3()
% clear all;
close all;

dir_save_figure = ['D:\Arseny\S1ALM_revision_figures\'];
filename = 'Sup_REV1_DetailedBehavior3';
...
ylabel('Performance (%)');
fn_behav_bar_anova_rev(b_query,names_trials,color_bars,x, behav_param);
text(xl(1)-diff(xl)*0.3, yl(1)+diff(yl)*1.1, 'a', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
xlim(xl);
...
hold on
yl = [0,0.5];
fn_behav_bar_anova_rev_reaction_time(b_query,names_trials,color_bars,x, behav_param);
