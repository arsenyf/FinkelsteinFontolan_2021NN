function  POUT = fn_plotProjection_for_figure2_single (Param, rel, key, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag)
hold on;
...
smooth_time = Param.parameter_value{(strcmp('smooth_time_proj_stimulus_single_session',Param.parameter_name))};
...
for itype= 1:1:numel(trial_types)
    ...
    plot(time,POUT.p(itype,:)+random_jitter,'Color',POUT.rgb(itype,:) ,'LineWidth',1,'LineStyle',line_style);
end
