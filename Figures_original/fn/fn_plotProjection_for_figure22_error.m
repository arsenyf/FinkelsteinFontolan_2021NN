function  POUT = fn_plotProjection_for_figure22_error (Param, rel, key, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag)
hold on;


min_num_trials_projected = Param.parameter_value{(strcmp('min_num_trials_projected_error_or_ignore',Param.parameter_name))};
...
proj_min = 0;
proj_max = 0;
Proj=struct2table(fetch(rel & 'outcome="miss"','*','ORDER BY trialtype_plot_order DESC'));
...
for itype= 1:1:numel(trial_types)
        P = Proj(strcmp(trial_types{3},Proj.trial_type_name),:);
