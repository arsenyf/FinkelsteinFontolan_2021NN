...
colr{4}=fetch1(ANL.TrialTypeGraphic &  'trial_type_name="r"','trialtype_rgb');

y = repmat(sin(x),4);
plot(linspace(-2.5, -2.1, 4*numel(x)),y*(factor*3)+yl(2),'Color',colr{4},'LineWidth',0.35,'Clipping','off')

y = repmat(sin(x),1);
plot(linspace(-2.5, -2.4, numel(x)),y*(factor)+yl(2),'Color',colr{1},'LineWidth',0.5,'Clipping','off')
plot(linspace(-1.6, -1.5, numel(x)),y*(factor)+yl(2),'Color',colr{2},'LineWidth',0.5,'Clipping','off')
plot(linspace(-0.8, -0.7, numel(x)),y*(factor)+yl(2),'Color',colr{3},'LineWidth',0.5,'Clipping','off')
