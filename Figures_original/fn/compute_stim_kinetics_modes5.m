function [OUT, GP] = compute_stim_kinetics_modes5 (rel,outcome,Param, colr, flag_normalize_modes, key)
...
smooth_time = Param.parameter_value{(strcmp('smooth_time_proj_choice_kinetics',Param.parameter_name))};
smooth_bins=ceil(smooth_time/psth_time_bin);



k_outcome.outcome=outcome;
Proj=struct2table(fetch(rel & k_outcome,'*','ORDER BY session_uid DESC'));
...
for itype= 1:1:numel(trial_types)
    ...
    if size(P,1)>0 %average across sessions
        ...
        for is=1:1:size(P,1)
            ...
            Proj_left_temp=fetch1(rel & kkk & 'trial_type_name="l"' & 'outcome="hit"','proj_average');
            Proj_left_temp = movmean(Proj_left_temp ,[smooth_bins 0], 2,'omitnan', 'Endpoints','shrink');
            min_l=nanmin(Proj_left_temp(time>-4 & time<0));
%             min_l=0;
            ...
            Proj_x(is,:) = movmean(P.proj_average(is,:) ,[smooth_bins 0], 2,'omitnan', 'Endpoints','shrink');
            
            Proj_x(is,:) = (Proj_x(is,:) -  Proj_left_temp)./(max_r - min_l);       
