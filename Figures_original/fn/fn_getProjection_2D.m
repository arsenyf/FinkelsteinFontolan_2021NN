function  POUT = fn_getProjection_2D (Param, rel, key_mode, trial_type_substract,  flag_normalize_modes)
...
% if strcmp(key_mode.outcome,'miss')
%     trial_types=trial_types(contains(trial_types,'Mini'));
% end
...
POUT.p = POUT.p- nanmean(POUT.p(:,1:2),2); %removing baseline for each trial_type
...
%     POUT.p = POUT.p./nanmax(POUT.p(:));
    POUT.p = POUT.p./nanmax(Proj_right_hit(time>-4 & time<0));
