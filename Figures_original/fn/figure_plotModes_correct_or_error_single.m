function [xl, yl] = figure_plotModes_correct_or_error_single (rel,Param, xlabel_flag, title1, title2,title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, key,  xl, line_style, stim_time_flag)
...
% PROJ = fn_plotProjection_for_figure_correct_or_error2 (Param, rel, key_mode, trial_type_substract, ylab,mode_names_titles{1}, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type);
PROJ = fn_plotProjection_for_figure_correct_or_error_single (Param, rel, key_mode, trial_type_substract, ylab,mode_names_titles{1}, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type);
...
%     text ((xl(1)-diff(xl)*0.23), (yl(1) + diff(yl)*0.5), sprintf('%s mode\n         (a.u.)',mode_names_titles{1}),'Fontsize', 7,'Rotation',90,'VerticalAlignment','middle','HorizontalAlignment','center');
    text ((xl(1)-diff(xl)*0.2), (yl(1) + diff(yl)*0.5), sprintf('%s mode (a.u.)',mode_names_titles{1}),'Fontsize', 7,'Rotation',90,'VerticalAlignment','middle','HorizontalAlignment','center');
end
...
if xlabel_flag==1
    ...
    set(gca,'Xtick',[-2 0],'TickDir','out','FontSize',6);
else
    set(gca,'Xtick',[-2 0],'TickDir','out','FontSize',6);
end
