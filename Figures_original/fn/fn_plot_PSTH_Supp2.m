...
xl=[-2.8 0];
% if strcmp(outcome, 'notignore')
%     PSTH = PSTH(~strcmp('ignore', PSTH.outcome),:);
% else
...
smooth_time = 1*Param.parameter_value{(strcmp('smooth_time_cell_psth',Param.parameter_name))};
...
if xlabel_flag==1
    set(gca,'FontSize',7,'Xtick',[-2.5 -1.6 -0.8 0],'TickDir','out');
    text(xl(1)+diff(xl)*0.5, yl(1)-diff(yl)*0.25,'Time to Go cue (s)', 'FontSize',7,'HorizontalAlignment','center');
else
    set(gca,'FontSize',7,'Xtick',[-2.5 -1.6 -0.8 0],'XtickLabel',[],'TickDir','out');
end
