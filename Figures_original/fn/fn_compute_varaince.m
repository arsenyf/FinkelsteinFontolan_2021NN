...
for i_s = 1:1:numel(session_uid)
    ...
    for i_m = 1:1:numel(mode_names)
        ...
        proj_l(i_m,:) = movmean((fetch1(rel_Proj & k_s & k_proj & k & 'trial_type_name="l"','proj_average')),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
        proj_r(i_m,:) =  movmean((fetch1(rel_Proj & k_s & k_proj & k & 'trial_type_name="r"','proj_average')),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
