function [xl, yl] = figure_plotModes_correct_or_error_suppl_negative (rel,Param, xlabel_flag, title1, title2,title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, key,  xl, line_style, stim_time_flag)
...
PROJ = fn_plotProjection_for_figure_correct_or_error3_negative (Param, rel, key_mode, trial_type_substract, ylab,mode_names_titles{1}, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type);
...
if stim_legend_flag==1
    if isfield(key,'trialtype_flag_full')
        ...
        plot(linspace(-2.5, -2.1, 4*numel(x)),y_full*(factor)+yl(2)*1.15,'Color',colr{1},'LineWidth',0.35,'Clipping','off')
        plot(linspace(-1.6, -1.2, 4*numel(x)),y_full*(factor)+yl(2)*1.15,'Color',colr{2},'LineWidth',0.35,'Clipping','off')
        plot(linspace(-0.8, -0.4, 4*numel(x)),y_full*(factor)+yl(2)*1.15,'Color',colr{3},'LineWidth',0.35,'Clipping','off')
    else
        factor=diff(yl)/6;
        ...
        plot(linspace(-2.5, -2.1, 4*numel(x)),y_full*(factor)+yl(2)*1.15,'Color',colr{1},'LineWidth',0.35,'Clipping','off')
        plot(linspace(-1.6, -1.5, numel(x)),y_mini*(factor/3)+yl(2)*1.15,'Color',colr{2},'LineWidth',0.5,'Clipping','off')
        plot(linspace(-0.8, -0.7, numel(x)),y_mini*(factor/3)+yl(2)*1.15,'Color',colr{3},'LineWidth',0.5,'Clipping','off')
        plot(linspace(-2.5, -2.4,  numel(x)),y_mini*(factor/3)+yl(2)*1.15,'Color',colr{4},'LineWidth',0.5,'Clipping','off')
    end
end