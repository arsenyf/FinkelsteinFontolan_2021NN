function Fig_Slope2_regularmice() % MATLAB R2017a
close all;
...
rel_choice_full =(EXP.Session * EXP.SessionID * ANL.ProjTrialAverageBaseline4 * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction * ANL.SessionGrouping * ANL.TrialTypeStimTime) & k & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0') -  ANL.ExcludeSession;
xlabel_flag=1;
ylabel_flag=1;
y_lims = [-0.15,1];
stim_legend_flag=1;
...
rel_temp =(EXP.Session * EXP.SessionID * ANL.ProjTrialAverageBaseline4 * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction * ANL.SessionGrouping * ANL.TrialTypeStimTime)  & k & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0') -  ANL.ExcludeSession;
...
stim_legend_flag=0;
outcome='miss';
key_trial_type.trial_type_name='l_-0.8Mini';
line_style=':';
[~, ~] =figure_plotModes_correct_or_error (rel_choice_full ,  Param, xlabel_flag, title1, title2, title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, k,[],line_style,0);
set(gca,'Ytick',[ y_lims(1) y_lims(2)]);
% text(xl(1)-diff(xl)*0.4 , yl(1)+diff(yl)*1.25, 'h', ...
