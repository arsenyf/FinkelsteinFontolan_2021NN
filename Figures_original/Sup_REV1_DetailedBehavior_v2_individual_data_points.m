...
filename = 'Sup_REV1_DetailedBehavior_v2_individual_data_points';
...
fn_behav_bar_anova_rev_individual_mice_data_points(b_query,names_trials,color_bars,x, behav_param);
...
xlabel(sprintf('Time of distractors (s) \n relative to Go cue'));
fn_behav_bar_anova_rev_individual_mice_data_points(b_query,names_trials,color_bars,x, behav_param);
text(xl(1)-diff(xl)*0.3, yl(1)+diff(yl)*1.5, 'e', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
xlim(xl);
