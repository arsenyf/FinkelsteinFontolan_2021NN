...
filename = 'Fig4_low_vs_high_ramping';
...
% rel_Switch = (ANL.SwitchProbabilityHighvsLow11 * EXP.SessionID * EXP.SessionTraining *ANL.SessionGrouping *  ANL.IncludeSession - ANL.ExcludeSession ) & k & (EXP.SessionTask &'task="s1 stim"');
rel_Switch = (ANL.SwitchProbabilityHighvsLow11* EXP.SessionID * EXP.SessionTraining *ANL.SessionGrouping) & k & (EXP.SessionTask &'task="s1 stim"') & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0');
...
% k.trial_type_name='l_-1.6Full';
% 
% rel_Proj = ( ANL.ProjTrialNormalized333*EXP.SessionID*EXP.TrialName & key) & (EXP.BehaviorTrial * EXP.TrialName & k & 'outcome!="ignore"' & key & sprintf('num_units_projected>=%d', min_num_units_projected) &  (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0'));
% 
% 
%   Param = struct2table(fetch (ANL.Parameters,'*'));
%             time = Param.parameter_value{(strcmp('psth_t_vector',Param.parameter_name))};
%             psth_time_bin = Param.parameter_value{(strcmp('psth_time_bin',Param.parameter_name))};
%             smooth_time = Param.parameter_value{(strcmp('smooth_time_proj_single_trial_normalized',Param.parameter_name))};
...
% all_projs_mean = rescale(all_projs_mean);
% 
...
% 
% 
% y_mean1=nanmean(all_projs(idx_low,tidx));
% y_stem1=nanstd(all_projs(idx_low,tidx))./sqrt(size(all_projs(idx_low,:),1));
% 
% y_mean2=nanmean(all_projs(idx_high,tidx));
...
% set(gca,'FontSize',7,'Xtick',[-2.6 -1.6],'XtickLabel',[-1 0],'Ytick',yl);
