function Sup5_Choice_Mode_ALM_full_distractor2() % MATLAB R2017a
close all;
...
compute_stim_kinetics_modes5 (rel_choice_full, outcome, Param, colr, flag_normalize_modes, key);

xl=[0 0.8];
xlim(xl);
yl = [-0.12 0.12];
ylim(yl);
set(gca,'FontSize',7,'Ytick',[-0.1 0 0.1], 'Xtick',[0 xl(2)], 'XtickLabel',[0 0.8]);
...
rel = ( ANL.ProjTrialNormalizedMedianNormalized11 * EXP.SessionID * EXP.BehaviorTrial * EXP.TrialName* EXP.SessionTraining  * ANL.SessionGrouping & key & sprintf('num_units_projected>=%d', min_num_units_projected)) & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0')- ANL.ExcludeSession;
