function Sup4_Stimulus_vS1_and_ALM__full_distractors() % MATLAB R2018ba
close all;
...
filename = 'Sup4_Stimulus_vS1_and_ALM__full_distractors';
...
position_x1(1)=0.12;
position_x1(2)=position_x1(1)+horizontal_distance1*1.1;
...
position_x1(6)=position_x1(5)+horizontal_distance1*1.2;
...
text(-3,110,'Distractor-trained mice, strong distractors','HorizontalAlignment','Left','FontSize',9,'Color', legends{1}.colr,'fontweight','bold' )
text(-1.4,85,'Delay','FontSize',6);


% Cell 2
key_u.unit_uid=863; %912; 823; 863 1669 1691, 1751
...
% vS1 Quantification
axes('position',[position_x1(4), position_y1(1), panel_width1*0.75, panel_height1]);
...
%Cell 1
key_u.unit_uid=314; %348  good;  %3726 not good
axes('position',[position_x1(1), position_y1(2), panel_width1, panel_height1]);
xlabel_flag=1; ylabel_flag=1; stim_legend_flag=1; unit_counter=unit_counter+1; cell_title=''; cell_label='B';
fn_plot_PSTH_Supp2 (Param, 'response', 1, 1, [], 1, key_u, xlabel_flag, ylabel_flag, stim_legend_flag, unit_counter, cell_title, cell_label);


% Cell 2
key_u.unit_uid=3674; %228 ok %284 not good % 287 great negative %314 very good % 294 good negatuve %3674 good negative
...
% vS1 Quantification
axes('position',[position_x1(4), position_y1(2), panel_width1*0.75, panel_height1]);
...
text(-3,yl(1)+diff(yl)*1.35,'Response size','FontSize',7);
text ((xl(1)+diff(xl)*0.5), (yl(1) + diff(yl)*-0.25), sprintf('Time to Go cue (s)'),'Fontsize', 7,'HorizontalAlignment','center');
set(gca,'Xtick',[-2 0], 'Ytick',[yl(1) 0 yl(2)], 'YColor',colr,'Fontsize', 7)
...
plot_r_flag=1;
flag_normalize_modes=1;
axes('position',[position_x1(6), position_y1(2), panel_width1*0.75, panel_height1]);
hold on;
colr = legends{3}.colr;
yl=[0,1.4];
ylim(yl);
signif_display_offset =-0.3;
compute_stim_amp_full_pairwise2_rev_regression (rel,  Param, colr, flag_normalize_modes, plot_r_flag, yl, signif_display_offset, k);
