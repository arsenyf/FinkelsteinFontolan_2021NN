function  POUT = fn_plotProjection_for_figure_correct_or_error_timeshifted (Param, rel, key_mode, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type)
hold on;


min_num_trials_projected_correct = 5; %Param.parameter_value{(strcmp('min_num_trials_projected_correct',Param.parameter_name))};
min_num_trials_projected_error_or_ignore = 5 %Param.parameter_value{(strcmp('min_num_trials_projected_error_or_ignore',Param.parameter_name))};
...
%     idx_pure_r = find(strcmp('r',POUT.trial_types));
    ...
    max_r=nanmax(Proj_right_hit(time>-4 & time<0));
    if max_r<0
...
for itype= 1:1:numel(trial_types)
    ...
    if ~exist('line_style')
        line_style='-';
            ...
            if contains(trial_types{itype},'-1.6')
                temp_time = time + ( (-2.5)-(-1.6) );
            end
                
    if line_style==':'
            plot(temp_time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',0.75,'LineStyle',line_style);
    else
    plot(temp_time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',0.75,'LineStyle',line_style);
    end
end
