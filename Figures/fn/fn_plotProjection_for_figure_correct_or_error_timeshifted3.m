function  POUT = fn_plotProjection_for_figure_correct_or_error_timeshifted3 (Param, rel, key_mode, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type)
hold on;
...
%
% Proj_right=struct2table(fetch(rel & 'trial_type_name="r"' & 'outcome="hit"','*'));
...
for itype= 1:1:numel(trial_types)
    ...
    if contains(trial_types{itype},'-1.6')
        ...
        if contains(trial_types{itype},'-0.8')
        temp_time = time + ( (-2.5)-(-0.8) );
    end
    if line_style==':'
        plot(temp_time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',1.25,'LineStyle',line_style);
    else
