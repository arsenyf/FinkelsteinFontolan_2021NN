function  POUT = fn_plotProjection_for_figure_correct_or_error_single (Param, rel, key_mode, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type)
hold on;
...
smooth_time = Param.parameter_value{(strcmp('smooth_time_proj_choice_single_session',Param.parameter_name))};
...
for itype= 1:1:numel(trial_types)
    ...
    plot(time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',1.1,'LineStyle',line_style);
    end
end
