function Fig_Slope1() % MATLAB R2017a
close all;
...
rel_temp =(EXP.Session * EXP.SessionID * ANL.ProjTrialAverageBaseline3 * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction * ANL.SessionGrouping * ANL.TrialTypeStimTime)  & k & (ANL.IncludeSessionSelectivity2 & 'num_delay_selective_units>=0' & 'num_sample_selective_units>=0') -  ANL.ExcludeSession;
...
% xdat = [0 0 0.4 0.4];
% fill([-1.6 + xdat], ydat, [0.6 0.85 1], 'LineStyle', 'None');
[~, ~] =figure_plotModes_correct_or_error_timeshifted2 (rel_temp ,  Param, xlabel_flag, title1, title2, title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, k, [],line_style,1);
...
key_trial_type.trial_type_name='l_-1.6Full';
line_style=':';
[xl, yl] =figure_plotModes_correct_or_error_timeshifted2 (rel_choice_full ,  Param, xlabel_flag, title1, title2, title3, ylabel_flag,  mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, key_trial_type, stim_legend_flag, k,[],line_style,0);
set(gca,'Ytick',[0 1]);
% text(xl(1)-diff(xl)*0.4 , yl(1)+diff(yl)*1.25, 'h', ...
%     'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
